    ...
    useEffect(() => {
        const getMovies = async () => {
            ...
            try {
                ...
                const response = await movieApi.getMoviesList(movieType.trending, params);
                ...
                movieItems.map((movie, index) => {
                    ...
                    return (
                        <SwiperSlide key={index}>
                            {({isActive}) => (
                                <HeroSlideItem movie={movie} className={`${isActive ? "active" : ''}`}/>
...
const HeroSlideItem = props => {
    ...
    const background = image_API.originalImage(item.backdrop_path ? item.backdrop_path : item.poster_path);
